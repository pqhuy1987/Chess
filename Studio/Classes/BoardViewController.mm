...
- (void)loadView {
   ...
   if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
      ...
      [activityIndicator release];
       ...
       rootView = [[RootView alloc] initWithFrame: appRect];
       appRect.origin = CGPointMake(0.0f, 0);
       contentView = [[UIView alloc] initWithFrame: appRect];
       
       UIGraphicsBeginImageContext(contentView.frame.size);
       [[UIImage imageNamed:@"bg"] drawInRect:contentView.bounds];
      ...
      UIToolbar *toolbar =
         [[UIToolbar alloc]
            initWithFrame: CGRectMake(0.0f, self.view.frame.size.height-44, 320.0f, 64.0f)];
      [contentView addSubview: toolbar];
